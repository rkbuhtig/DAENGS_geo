...
|---|---|---|---|---|
| 웹 인터랙티브 지도 (`/facility-map`) | **NAVER Dynamic Map** | 키 입력 후 활성 | 키 누락·SDK 로드 실패 시 OSM 개발 폴백 | canonical Place 검색과 마커를 브라우저에서 검증 |
...
4. **실험 폴백은 제품 폴백과 다르다.** OSM은 `/facility-map`에서 키 없이 UI를 검증하기 위한 장치다.
...
GET http://127.0.0.1:8000/facility-map
                         NAVER 배지와 실제 지도, canonical Place 검색·마커 확인
