...
작성일: 2026-09-05. 상태: 실제 프로필·개별 평가 PR 병합, 개발 서버와 Kotlin API 연결 검증 완료.
실기기 검증과 운영 기본 화면 전환은 남아 있다.

최신 대화 대조 및 리뷰 기록: [합의·구현·리뷰 현황](place-search-session-decisions.md).
앱 리뷰 수정 #149·#150·#152·#154도 병합됐다. 아래 단계별 구현 기록은 당시 이력이며,
현재 남은 작업은 최신 현황의 미검증/후속 기능 목록을 따른다.

최신 검증: [프로필 시설 검색 실제 연결 확인](place-search-live-verification.md).
개발 서버는 새 계약을 지원하지만 출시 도메인의 dogs echo는 미지원으로 관측했다.
실제 계정 종단 확인은 아직 완료하지 않았다.
...
- 프로필 기준: [반려견 선택 계약 v2](place-search-profile-contract.md). v1은 초기 제안 기록이다.
...
- [x] 기존 PlacesScreen, PlaceDiscoveryPanel, PlaceCard, PlaceCardPresentation, PlaceModels와 상태 흐름 확인.
- [x] 저장 응답을 사용하는 개발용 데이터 공급자를 두고 화면 컴포넌트와 검색 상태 분리.
- [x] 검색창 모드 전환, 2행 카테고리, 필터 줄, 결과 헤더, 서랍식 카드 이식.
- [x] restrictions·원문·출처·기준일·미상 정보를 표시 모델까지 보존.
- [x] 지도 선택·카드 이동·단일 펼침 연결 (코드/상태 검증).
- [ ] 실기기에서 지도 이동·위치 튐·중첩 스크롤·터치 확인.
- [x] 로딩·빈 결과·오류·권한 없음·미지원 데이터 상태 구현.
...
- [x] 최신 API의 장소명·주소 검색, 지역 해석, 중심·반경, 업종, 전체보기 지원 범위 조사표 작성.
- [x] 현재 지원 요청·응답 계약 확정 및 앱 연결. 지역명/주소 해석은 미구현으로 구분.
- [x] 일반 검색 제출, 카테고리·직접 조건 변경, 지도 지역 재검색을 공통 요청 경로에 연결.
- [x] 전체보기 중복 제거·통합 정렬·종류별 상한과 잘림 표시·결과 수 의미 확정 (페이지 탐색 미구현).
- [x] 주차 필수 조건과 주차 우선 정렬을 구분. 미지원 업종에 적용하지 않음.
- [x] 역순 응답·검색 실패·재시도·결과 잘림 및 지도/카드 키 연결 코드 검증.
- [ ] 실기기 지도·전화·길찾기 및 지역명/주소 검색 후속 구현·검증.
...
- [앱 #147](https://github.com/SAJOYO/DAENGS_APP/pull/147): 실제 이름 복수 선택, 기본 선택 없음,
  계정/삭제/배웅/실패 상태, 프로필 변경 재평가, 카드 개별 평가와 추가 조건 원문/출처.
- [서버 #253](https://github.com/SAJOYO/DAENGS_dev/pull/253): 단일 계약 호환을 유지하는 dogs 값 스냅샷,
  개별 평가/echo, 프로필 updated_at 노출, 확정 불일치보다 미상을 먼저 반환하던 평가 보완.
- 사용자 합의에 따라 복잡한 마릿수 자동 판정·전체 입장 가능 집계·자동 결과 제외는 구현하지 않는다.
  [선택 계약 v2](place-search-profile-contract.md)의 현행 계약과 초기 제안을 구분한다.
- 앱 APK 빌드·관련 테스트 135개 통과 후 묶음 간 버전 검사를 추가하고 관련 11개 재검증.
  서버 관련 101개 통과 후 버전 노출 테스트를 추가하고 프로필 19개 재검증.
  당시 남은 항목 중 개발 서버 배포는 이후 완료했다([연결 검증](place-search-live-verification.md)).
  DB 트리거 실측·실제 계정/실기기·release 전환은 이 기록으로 완료 판정하지 않는다.
...
초기 권장 단위는 조사 문서에 기록했다. 이후 합의한 구현 범위는 위 기록과 선택 계약 v2를 따른다.

- [x] 프로필 API·인증·소유권·버전 계약 코드 확인, 실제 이름과 ID로 복수 선택 연결.
- [x] 기본 선택·선택 유지 범위·프로필 삭제/변경·프로필 없음/실패 정책 확정.
- [x] 선택된 각 반려견의 평가와 이유 반환 구현. 전체 입장 가능 집계는 범위에서 제외.
- [x] 크기·체중과 추가 조건, 동반 마릿수 제한을 구분하고 미상 처리 정책 확정.
- [x] 프로필 변경 후 결과 갱신 구현.
- [ ] 향후 직접 신체 조건/AI 제안 연결 시 프로필과 충돌 표시 설계·구현.
- [x] 카드에서 어떤 반려견에게 어떤 제한이 적용되는지 확인 가능하게 연결.
- [ ] 실제 로그인 계정으로 프로필 읽기·수정·검색 종단 검증.
...
앱 PR #143·#145·#146은 병합되었다. 3단계 후속 앱/서버 PR과 검증 범위는 위 구현 기록을 따른다.
