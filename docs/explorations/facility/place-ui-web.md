...
일반 검색에는 별도 모드 안내를 표시하지 않고 AI 모드만 표시한다. 반려견 선택은
거리·적용 조건·설정 버튼과 같은 필터 줄에 둔다. 하단은 '내 주변 장소' 제목 대신
'카페 14곳'처럼 업종·결과 수와 정렬 선택만 표시한다. 정렬 선택과 주차 선호는 같은 상태를 공유한다.
...
이는 웹 검토판의 구현 기록이다. 이후 앱 #149에서 합의한 현재 표시 규칙은
명시적인 `dog_ok=false`를 우선하는 것이다([최신 현황](place-search-session-decisions.md)).
