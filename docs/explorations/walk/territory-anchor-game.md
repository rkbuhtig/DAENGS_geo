...
선별한 중립 지점이다. 2026-09-03 로컬 적재본은 480,144개다.
...
원본 보안등은 도심에서 지나치게 조밀하므로 `anchor-hex:115:q:r` 셀마다 하나만 남긴다.
서울 위도에서 반지름 115 Web Mercator 단위는 실제 약 91m이고 이웃 셀 중심 간격은 약
158m다. 다만 선택점은 셀 중심이 아니라 실제 원천 좌표다.
...
외부 identity는 DB의 `BIGSERIAL anchor.id`보다 `(source, cell)` 형태의 안정 참조를 우선
검토한다. 재적재나 DB 재구축이 게임 기록의 identity를 바꾸면 안 된다.
