...
**최신 제품 결정:** [기획 경위와 서술 기준](narrative-design/README.md)을 먼저 읽는다.
특별한 순간은 각각 별도 장면이며 사용자 자유 서술에 AI가 관여하지 않는다. 공간·환경만
보조하는 예외다. 아래 ‘같은 배경 조립’과 유사 행위 승격은 당시 구현의 설명이며,
자유 서술에 동선·생성 문장을 붙이거나 기기 관측을 반려견 행동으로 바꾸는 현재 지시가 아니다.
...
당시 구현 방향은 **사람이 남긴 행위 기록을 중심에 두고 시스템은 배경을 붙이는 것**이다.
