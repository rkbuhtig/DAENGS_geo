...
작업 재개는 **[산책 장면과 일기 생성 기획](plan.md)**과
**[기획 경위·서술 기준·실험·인계 묶음](narrative-design/README.md)**에서 시작한다.
상세 묶음에는 대화의 변화, 특별한 순간의 서술 불개입, 행동 핀의 가까운 시선,
어디서·누가·무엇을·어떻게·환경 α, 실제 지정 API 실험의 중단, 모델 출력 원문을 연결했다.
2026-09-12부터 장면 하나를 독립된 서브 일기로 완성하는 것이 우선이다.
전체 세션 이해·장면 순회·앞 장면 재검토는 생성의 필수 단계에서 제거했다.
전체 일기는 완성된 장면과 원본·추가 자료를 별도로 읽는 후속 생성이다.
기획은 채택했으며 기존 코드의 입력·서술·제품 연결 반영은 남아 있다.

입력의 값·계산 정의·확보 범위와 공간 경향 추출 정책은 **[자료 카탈로그](evidence-catalog.md)**를 읽는다.
이 목록 전체가 장면 작성 입력은 아니다. 시간·순서 등 시스템 판정용 자료와 서술 재료의
경계는 기획 문서를 따른다.

## 재사용할 구현과 별도로 확인할 부분

[장면 중심 생성과 공통 배경 조립](scene-pipeline.md)이 기존 Geo 오프라인 구현 경로다.
...
카드는 스탬프 버전을 참조한다. 기존 모델 입력의 시간·앞뒤 관계와 선택/작성 계약을
새 기획에 맞췄다는 뜻은 아니다.

특히 ‘특별한 순간’ 자유 서술을 일반 생성과 같은 배경 조립·본문 재작성에 넣는 것은 최신
결정과 다르다. 기록마다 별도 장면을 보장하고 공간·환경만 별도로 제공하는 경계는
[유형별 서술 기준](narrative-design/02-narration-contract.md)을 따른다.

## 이전 실험과 연구

아래 자료는 당시 입력·출력·실패를 재현하기 위한 기록이다. 현재 제작 절차로 이어받지 않는다.
겹치는 이전 아키텍처·청사진 본문은 삭제했다. 필요하면
[변경 전 기획과 문서](https://github.com/rkbuhtig/DAENGS_geo/tree/0e4942e18afc8e373fc1626e7c3d8b2dc7ac957f/docs/explorations/walk/diary)를 확인한다.
...
첫 [실제 구성 A/B 결과](../../../research/2026-09-08-event-scene-gemini-ab.md)는 구조 통과 0/6이었다.
