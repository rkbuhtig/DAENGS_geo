...
현재 제품 기획은 [장면 우선 생성](plan.md)이다. 아래는 기존 오프라인 재료 조립 구현이며,
시간·연속성은 시스템 판정에 사용한다. 작성 입력과 완성 본문의 새 경계까지 구현한 것은 아니다.
특별한 순간을 각각 보존하고 공간·환경만 보조하며 본문에 AI가 관여하지 않는 후속 결정은
[상세 서술 기준](narrative-design/02-narration-contract.md)을 따른다. 아래 공통 조립을 그 예외까지
반영한 최신 서술 계약으로 읽지 않는다.
