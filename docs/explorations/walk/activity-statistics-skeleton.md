...
구현하기 위한 설계 제안이다. 이번 문서 작업은 새 세션 서비스·통계 테이블·API가 구현됐다는
뜻이 아니다. 기존 게임 구현/이관은 [인수인계](territory-game-handoff.md), 순위·칭호는
...
아래는 구현할 논리 구조다. 현재 테이블/클래스가 있다는 뜻은 아니며, 실제 마이그레이션에서
기존 키 및 삭제 정책에 맞춰 제약과 타입을 정한다.
...
통과해야 저장 skeleton이 완성된다. 현재 문서는 그 설계이며, 이 시나리오의 실행 검증 결과는 아니다.
...
이번 변경은 S1의 설계 제안이다. S2 이후 코드와 마이그레이션은 아직 만들지 않았다.
