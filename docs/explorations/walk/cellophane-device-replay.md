...
이 replay의 목적은 제품 저장을 새로 시작하는 것이 아니라, 서버 검증과 같은 canonical 경로를
실제 Android export에 대어 PaintSpec을 비교할 측정 근거를 만드는 것이다.
...
다음 저장·overlay 단계로 넘어가는 조건은 실제 산책에서 네 후보를 비교해 canonical
radius/profile을 선택할 근거가 생기는 것이다. 그 전에는 DB migration이나 Android overlay를 만들지 않는다.
...
이 측정은 **실기기 GPS 지터 근거가 아니다.** 다만 Android 수집 경계를 실제로 통과해 replay
