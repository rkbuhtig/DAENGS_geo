...
## 6차 잔여 정리와 최종 대조 (2026-09-07)

개발용 SQL은 `seeds/dev_seed.sql`, 스키마 수명·과거 SQL 안내는 `alembic/README.md`로
옮긴다. SQL 내용과 Alembic 리비전은 그대로 유지한다. 현재 저장소 지도·실행 명령·실험
목록을 새 소유 위치에 맞추고, 과거 연구·결정의 옛 경로는 당시 기록으로 보존한다.

승격 원장의 source/target 커밋은 바꾸지 않는다. 기존 app/discovery와 tests/place,
tests/discovery 아래에서 추적하던 검토 파일 3개의 새 경로만 추가해 추적 누락을 막는다.
이 경로는 기존 검토 근거를 이어 보는 범위이며 검토 도구의 운영 채택 선언이 아니다.

독립 CLI·설정 호환 필드는 사용처를 확인해 유지한다. 실험 상태만으로 일괄 삭제하지 않고
현재 소비자·재현 경로가 있는 실험은 보존한다. 단계별 변경과 0차 대비 검증·남은 실행
환경 제약은 [구조 정리 결과](../research/2026-09-07-repository-reorganization.md)에 기록한다.
