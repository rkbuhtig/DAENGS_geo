...
| 57 | **산책 데이터는 좌표 함유량으로 층을 가른다.** 연속 궤적은 finish 이후 서버에 남기지 않고(구현 사실 → 정책), 무좌표 집계(`WalkFacts`)는 장기 보관 후보, 좌표 동반 층(`MotionEventOccurrence`·`FacilityEncounter`)은 기본이 짧다. 삭제는 로컬 원본에서 시작해 서버로 가고(반대 순서는 재업로드로 무효화된다) 숨김은 만들지 않는다. 구체 일수가 서기 전까지 업로드는 개발 기기로 제한한다 ([문서](2026-08-25-walk-data-retention.md)) | "산책 기록을 저장할까요"는 답할 수 없는 질문이다 — 남는 것들의 민감도가 서로 다르다. 매일 같은 시각 같은 좌표의 정지는 집이고 산책은 현관에서 시작하므로, 궤적만 지우고 점은 괜찮다고 보면 틀린다. 실사용 데이터가 0이라 일수는 근거가 없어 정하지 않는다 — 근거 없는 숫자는 결정으로 읽힌다. 삭제도 층이다: 업로더가 "전부 재전송·추적 없음"이라 로컬 행을 남기면 재시도가 붙는 순간 지워진 세션이 되살아난다 | 2026-08-25 |
| 58 | **세션을 묶는 식별자는 프로필이 아니고, 우리가 만들지도 않는다.** `dog_id`는 역참조하지 않는 불투명 키이며 빌드/외부에서 주입받는다. 비어 있으면 업로드하지 않는다. 로컬 세션 행도 자기 `dogId`를 저장한다 ([문서](2026-08-25-subject-identity.md)) | 결정 #4가 금지한 건 프로필 소유이지 세션을 묶는 것이 아니다 — baseline은 개별 개에 붙어야 의미가 있다. 기기가 UUID를 만들면 재설치마다 새 개가 되고 조각난 baseline을 합칠 근거가 없다. 조용히 조각나는 것보다 업로드가 꺼져 있는 게 낫다. 행이 누구 것인지 모르면 그 subject의 삭제 요청을 로컬에서 실행할 수 없다 | 2026-08-25 |
