...
| 62 | **시설 검색은 반경 안을 다 준다 — `limit` 은 선택이고 기본이 없다.** 명시했을 때만 잘리고, 잘리면 `truncated` 로 알린다 | 상한 50 은 리스트 화면의 수치였는데 지도가 같은 API 를 쓰면서 조용히 누락됐다. 강남역 3km 용품 401 곳 중 50 곳만 나왔고, 크기 제약 시설이 그 밖에 있어 **필터가 일하는 것 자체가 안 보였다.** `/anchor/search` 가 `truncated` 를 주는 이유와 같다 — 조용히 자르면 "이 동네엔 이만큼뿐"으로 읽히고, 시설 표면에서 그 오독은 "우리 개가 갈 곳이 없다"가 된다. 부하는 카테고리(`kind`)로 나눠 부르는 쪽에서 푼다 | 2026-08-25 |
