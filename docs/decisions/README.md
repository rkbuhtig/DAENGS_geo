...
| # | 결정 | 이유 | 날짜 |
|---|---|---|---|
| 52 | 선호 태그의 의미 규칙과 부스트 순위 규칙을 `geo/ranking.py` 한 곳에 둔다 (이슈 #24) | 두 진입 경로가 각자 조립해서 같은 조건이 엔드포인트마다 다른 뜻이 됐다 — `/hospital` 은 순위 정책, `/places`·`/pharmacy` 는 표시용이라 `night=true` 가 순서를 안 바꿨다. 정책 조립은 도메인별로 달라도 되지만 같은 개념의 해석은 하나여야 한다 | 2026-08-25 |
| 53 | 조언·등급 판정은 응답 에이전트(RAG), 검색 실행은 이 레포. LLM 이 꺼져도 기능 집합은 줄지 않는다 ([문서](2026-08-25-agent-parallel-response.md)) | 판단 근거가 있는 쪽이 판단하고 데이터가 있는 쪽이 실행한다. 응급도를 우리가 흉내내던 문제의 해소이자, resolver 상황 정책을 밖으로 빼는 근거 | 2026-08-25 |
