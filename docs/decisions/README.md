...
| 54 | **산책 원본 fix 는 기기(Room/SQLite)에 저장한다.** 컬럼은 서버 `walk_fix` 계약 미러. 걸러진 선·거리·셀은 저장하지 않고 조회 시 파생한다. 서버 업로드 여부는 별도 결정으로 미룬다 | 필터 문턱값은 실기기 측정 전의 잠정값이라 바뀐다 — 걸러진 결과를 저장하면 오늘의 문턱값이 과거 산책에 박히고 재계산이 불가능해진다. 원본만 되돌릴 수 있는 선택지다. 보관·삭제는 기기 소유자 소관이라 별도 정책이 필요 없고, 이 결정은 업로드 결정을 막지 않는다 | 2026-08-24 |
