...
`hospital`과 `pharmacy`는 MOIS 인허가 기반 MedicalResolver가 존재 권위를 갖고, 그 밖의
kind는 KCISA/KTO FacilityResolver가 담당한다. 결과는 모두 `PlaceResult`로 변환되며 내부 DB
PK를 identity로 노출하지 않는다.
