...
- [ ] **KTO `acmpyTypeCd` 246행이 파싱되지 않는다** — 전구역 동반가능 125 · 일부구역 동반가능
      121. KTO 9,692행에서 `pet_allowed` 가 전부 NULL 인데 이 246행만 값이 있다.
      사실이 0인 층에 있는 유일한 우물이라 파싱할 값어치가 있다
      ([측정](research/2026-08-27-tag-material.md) §1)
- [ ] **`kind=shopping` 안에 약국이 섞여 있다** — 365열린약국·온누리약국 계열. KTO 가
      쇼핑(contenttypeid 38)으로 등록한 행이라 원천 분류상 오류는 아니지만(#65 는 원천
      category 를 보존한다), 사용자가 약국을 찾을 때 `kind=pharmacy` 만 보면 이 행들이
      안 보인다. 태그가 아니라 분류·표시 문제로 다룰 것
- [ ] **`pet_dog_ok` 는 63행에만 값이 있다** — 23,851행이 NULL 인 죽은 축이다.
      `evaluate_dog_access` 가 이 값을 읽으므로, 채울 원천이 없다면 축을 접을지 결정할 것
