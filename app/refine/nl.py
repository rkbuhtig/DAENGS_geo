...
_RULES: list[tuple[re.Pattern[str], Any]] = [
    ...
    (re.compile(r"관절|정형|다리|절뚝|슬개골|십자인대"), lambda m, s, ids: [ToolCall("set_specialty", {"tags": s.specialty + ["ortho"]})]),
    (re.compile(r"안과|눈이|백내장|눈\s*뿌옇"), lambda m, s, ids: [ToolCall("set_specialty", {"tags": s.specialty + ["eye"]})]),
    (re.compile(r"치과|이빨|치석|잇몸"), lambda m, s, ids: [ToolCall("set_specialty", {"tags": s.specialty + ["dental"]})]),
    (re.compile(r"피부|가려|긁"), lambda m, s, ids: [ToolCall("set_specialty", {"tags": s.specialty + ["derma"]})]),
    (re.compile(r"심장|호흡|숨"), lambda m, s, ids: [ToolCall("set_specialty", {"tags": s.specialty + ["cardio"]})]),
    ...
    (re.compile(r"(\d+)\s*분\s*(안|이내|내로|넘지)"), lambda m, s, ids: [ToolCall("set_walk_max", {"minutes": int(m.group(1))})]),
