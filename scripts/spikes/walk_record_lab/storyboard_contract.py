...
class Scene(StrictModel):
    ...
    reasons: list[Literal["session_boundary", "action", "note", "profile_change",
                          "session_speed", "distance_fill", "observation_gap"]] = Field(min_length=1)
...
def export_storyboard(artifacts, entries, selection, contexts):
    ...
    local_source = {"id": "local-walk", "provider": "walk-observation", "status": "known",
                    "captured_at": None, "url": None}
    scenes = []

    def append(identity, at_s, title, kind, text, reasons, route=None, context=None,
               end_s=None, movement=()):
        scene_id = "scene-"+fingerprint({"session": artifacts.scenario["session_id"],
                                         "identity": identity})[:24]
        sources = [local_source]
        facts = [{"id": scene_id+":fact", "kind": kind, "text": text,
                  "source_ids": ["local-walk"]}]
        if context:
            for source in context.get("sources", []):
                provider = source.get("source") or "unknown"
                sources.append({"id": "environment-"+provider, "provider": provider,
            ...
            for i, value in enumerate(context.get("facts", [])):
                provider = ("commerce" if value.startswith("좌표 반경") else
                            "parks" if "대표점" in value else
                            "rivers" if "시점 좌표" in value else None)
                refs = ["environment-"+provider] if provider else source_ids
                facts.append({"id": scene_id+f":environment:{i}", "kind": "environment",
                              "text": value, "source_ids": refs})
            if not context.get("facts"):
                facts.append({"id": scene_id+":coverage", "kind": "coverage",
                              "text": "이 구간을 설명할 환경 자료를 확인하지 못했어요. 시설이 없다는 뜻은 아니에요.",
                              "source_ids": source_ids})
        for i, m in enumerate(movement):
            facts.append({"id": scene_id+f":movement:{i}", "kind": "movement",
                          "text": f'관측 구간 {m["start_s"]:.0f}~{m["end_s"]:.0f}초의 평균 속도 '
                                  f'{m["mean_mps"]:.2f}m/s · 비교 기준 {m["baseline_mps"]:.2f}m/s',
                          "source_ids": ["local-walk"]})
        content = {"id": scene_id, "started_at": start+timedelta(seconds=at_s),
                   "ended_at": start+timedelta(seconds=end_s if end_s is not None else at_s),
                   "route": route, "reasons": reasons, "title": title,
                   "facts": facts, "sources": sources}
        revision = fingerprint({**content, "started_at": content["started_at"].isoformat(),
    ...
    for entry in entries:
        if not entry["accepted"]:
            continue
        append("entry:"+entry["id"], entry["elapsed_s"], entry["label"],
               "note" if entry["kind"] == "note" else "action",
               entry["note"] or entry["label"]+" 기록을 남겼어요.",
               ["note" if entry["kind"] == "note" else "action"],
               {"start_m": entry["accepted_distance_m"], "end_m": entry["accepted_distance_m"],
                "block_id": None} if entry["location"] is not None else None)
    for anchor in selection["anchors"]:
        ...
        title = ("평소와 다른 이동 구간" if "profile_change" in anchor["reasons"] else
                 "이동 속도가 달라진 구간" if "session_speed" in anchor["reasons"] else
                 f'경로 {round(anchor["route_m"])}m 주변')
        append(identity, anchor["elapsed_s"], title, "walk",
               f'산책 경로 {round(anchor["route_m"])}m 부근의 관측 지점이에요.',
               anchor["reasons"], {"start_m": anchor["route_m"], "end_m": anchor["route_m"],
                                   "block_id": anchor["block"]},
               contexts.get(anchor["id"]), movement=anchor["movement_evidence"])
    for gap in artifacts.computed.trail.gaps:
        ...
        append(f"gap:{a}:{b}", a, "위치 기록이 비어 있는 구간", "coverage",
               "이 사이의 이동 경로는 확인할 수 없어요.", ["observation_gap"], end_s=b)
    facts = artifacts.computed.facts
    append("end", artifacts.derived["truth_duration_s"], "산책 마무리", "walk",
           f"수용 이동거리 {round(facts.moving_distance_m)}m", ["session_boundary"])
    scenes.sort(key=lambda s: (s.started_at, s.id))
    payload = [s.model_dump(mode="json") for s in scenes]
    return StoryboardBundle(session_id=artifacts.scenario["session_id"], synthetic=True,
                            source_revision=fingerprint(payload), scenes=scenes)
