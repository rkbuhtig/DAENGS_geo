...
**폰에서 바로 보내기.** `지도 기능` 탭의 `산책 기록 보내기` 를 누르면 종료한 산책의 원본
JSON 을 공유 시트로 넘긴다 — 메일 · 메신저 · Drive 어디로든 간다. 옆에 대기 건수가 뜬다.

이게 아래 adb 경로보다 먼저인 이유: 산책은 개발 PC 에서 멀리 떨어져 하는 일이다. 공유가
없으면 export 는 앱 내부 저장소에 갇혀 **SDK 가 깔린 그 PC 에 USB 로 꽂아야만** 볼 수 있다.
